...
pedigree_pie <- pedigree_pie %>%
  complete(ascendency = seq(4, 262143, by=1)) %>%
  mutate(
    gen = case_when(
      ascendency == 1 ~ 1,
      ascendency %in% 2:3 ~ 2,
      ...
      ascendency %in% 8192:16383 ~ 14,
      ascendency %in% 16384:32767 ~ 15,
      ascendency %in% 32768:65535 ~ 16,
      ascendency %in% 65536:131071 ~ 17,
      ascendency %in% 131072:262143 ~ 18)) %>%
  group_by(gen) %>%
  ...
  select(-c(gen_count, ascendency)) %>%
  left_join(people, by=c("id" = "fid")) %T>%
